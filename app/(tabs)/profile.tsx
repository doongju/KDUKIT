...
import { deleteUser, getAuth, signOut } from 'firebase/auth'; // sendPasswordResetEmail 제거됨
import {
    arrayRemove,
    collection,
    deleteDoc,
...
// ✨ [추가됨] 새로 만드신 비밀번호 변경 모달 import (경로는 실제 파일 위치에 맞춰주세요)
...
const fetchBlockedUsers = useCallback(async (blockedIds: string[]) => {
    ...
    // ✨ [수정됨] 비밀번호 변경 버튼 핸들러 (이제 모달만 띄웁니다)
    ...
    const handleContactAdmin = () => {
        ...
        const adminEmail = "2124008@v.kduniv.ac.kr";
    ...
    return (
        ...
        >
            ...
            <ChangePasswordModal
                ...
                onClose={() => setChangePwModalVisible(false)}
            />
        </ScrollView>
    );
}
